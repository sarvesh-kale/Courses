...
int main (int argc, char* argv[])
{
	...
	// here we are doing 00000000
	Q[1] = bdd_zero(bddm); Q[3] = bdd_zero(bddm); Q[5] = bdd_zero(bddm); Q[7] = bdd_zero(bddm);Q[9] = bdd_zero(bddm);	
	Q[11] = bdd_zero(bddm); Q[13] = bdd_zero(bddm); Q[15] = bdd_zero(bddm);Q[16]=0;
	
	int assoc = bdd_new_assoc(bddm,Q,1);//last value non-zero in parameter list 
	bdd_assoc(bddm,assoc);
	//
	bdd f_substituted = bdd_exists(bddm,f);
	bdd g_substituted = bdd_exists(bddm,g);
	printf("Functions evaluated for 00000000 : \n");
