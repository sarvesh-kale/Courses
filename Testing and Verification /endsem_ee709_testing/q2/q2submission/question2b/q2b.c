...
int main(int argc ,char * argv[]) {
    ...
    bdd A = bdd_or(bddm,bdd_and(bddm,y1,y2),bdd_and(bddm,y2,y3));
    //preimage computation is there exists on y1,y2,y3 of A.X where X is
    ...
    bdd Q[5]={y1,y2,y3,0} ; //on these variable ext. quantification
    int assoc = bdd_new_assoc(bddm,Q,0);
    bdd_assoc(bddm,assoc);
    bdd preimage = bdd_exists(bddm,AX);
    bdd_print_bdd(bddm,preimage,NULL, NULL,NULL, stdout);
    return 0;
}
